...
We we take a sufficiently nice function $f(x)$ then we know that partial derivatives with respect to the coordinates commute:
...
For the future use we will need to define two more tensors:
- **Ricci tensor**: $R_{\mu\nu}=R_{\lambda\mu\,\,\,\nu}^{\,\,\,\,\,\,\lambda}$
- **Ricci scalar**: $R=g^{\mu\nu}R_{\mu\nu}$

### Riemann tensor for the two-sphere

We conclude this section with an explicit calculation of these tensors for the simple case of the two-sphere. The line element is
...
where $x^1=\theta$ and $x^2=\phi$. We can read off the elements of the metric tensor from this line element: $g_{\theta\theta}=R_0^2$ and $g_{\phi\phi}=R_0^2\sin^2\theta$. In a matrix form
...
First, we find all Christoffel symbols $\Gamma_{i\,\,j}^{\,\,k}$ for this metric everywhere

$$
\Gamma_{i\,\,j}^{\,\,k}=\frac{1}{2}g^{kl}(\partial_i g_{jl}+\partial_j g_{il}-\partial_l g_{ij})=\Gamma_{j\,\,i}^{\,\,k}
$$

where $i,j,k=1,2$. We calculate
...
&=\frac{1}{2}R^{-2}R^2 (-\partial_\theta \sin^2\theta)=-\frac{1}{2}2\sin\theta\cos\theta=-\sin\theta\cos\theta\\
...
\Gamma_{\theta\,\,\,\phi}^{\,\,\,\theta}&=\frac{1}{2}g^{\phi \phi}(\partial_\theta g_{\phi \phi}+\partial_{\theta}g_{\theta \phi}-\partial_\phi g_{\theta\phi})=\frac{1}{2}\left(\frac{1}{\sin^2\theta}\right)\left(\partial_\theta \sin^2\theta\right)=\frac{2\sin\theta\cos\theta}{2\sin^2\theta}=\frac{\cos\theta}{\sin\theta}=\Gamma_{\phi\,\,\,\theta}^{\,\,\,\theta}\\
...
$$
\Gamma_{\phi\,\,\,\phi}^{\,\,\,\theta}=-\sin\theta\cos\theta\,,\qquad\qquad \Gamma_{\theta\,\,\,\phi}^{\,\,\,\theta}=\Gamma_{\phi\,\,\,\theta}^{\,\,\,\theta}=\frac{\cos\theta}{\sin\theta}\,.
$$
...
Since $R_{ij\,\,\,l}^{\,\,\,\,\,k}$ is antisymmetric with respect to the first two indices, then we need to take $i\neq j$ to get a non-zero result. Next week we also see the the tensor $R_{ijkl}$ is antisymmetric with respect to the last two indices. Since the metric of the two sphere, it means that we need to take $k\neq l$ as well. Therefore the only non-trivial element of the Riemann tensor is $R_{\theta\phi\,\,\,\phi}^{\,\,\,\,\,\,\theta}$:
