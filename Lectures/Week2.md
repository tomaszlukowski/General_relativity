...
S=-mc^2\int dt \sqrt{1-\frac{\dot{\vec{x}}^2}{c^2}}=-mc^2\int dt \,\gamma\,
```
...
$$
S_{non-rel}=\int dt\left[\frac{m}{2}\dot{\vec{x}}^2-V(\vec{x})\right]
$$
...
Let us start by modifying the action [](non-rel-action) by adding a potential term, as we would do in the non-relativistic case:

$$
S=\int dt\left[ -mc^2\sqrt{1-\frac{\dot{\vec{x}}^2}{c^2}}-V(\vec{x})\right]=\int d\lambda\left[\sqrt{-\eta_{\mu\nu}\frac{dx^\mu}{d\lambda}\frac{dx^\nu}{d\lambda}}-V(x^\mu)\right],
$$
...
$$
S=\int dt\left[ -mc^2\sqrt{1-\frac{\dot{\vec{x}}^2}{c^2}}-q\,\phi(\vec{x})-q\vec{A}(\vec{x})\cdot\vec{\dot{x}}\right]
$$
...
S= -mc^2\int dt\sqrt{1+\frac{2\phi(\vec{x})}{c^2}-\frac{\dot{\vec{x}}^2}{c^2}}.
```
...
$$
S=\int dt \left[-mc^2 +\frac{m}{2}\vec{\dot{x}}^2-m\phi(\vec{x})+\ldots\right].
$$
...
$$
\boxed{\vec{y}=\vec{x}_2-\vec{x}_1}\quad \Rightarrow\quad  \boxed{\ddot{\vec{y}}=-\left(\vec{\nabla}\phi\Big|_{\vec{x}+\vec{y}}-\vec{\nabla}\phi\Big|_{\vec{x}}\right)}\quad\Rightarrow\quad \boxed{\ddot{y}_i=-\left(\partial_i\phi\Big|_{\vec{x}+\vec{y}}-\partial_i\phi\Big|_{\vec{x}}\right)\sim -\left(y^j\partial_j\partial_i\phi\Big|_{\vec{x}}+\mathcal{O}(y^2)\right)}
$$
