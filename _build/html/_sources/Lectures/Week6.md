...
Define covariant derivative of a vector $W^\mu$ with respect to $\lambda$ by

$$
\frac{DW^\mu}{D\lambda}=\frac{d W^\mu}{d\lambda}+\dot{x}^\rho \,\Gamma_{\rho\,\,\,\nu}^{\,\,\,\mu}W^\nu
$$

We can connect this definition to the definition of a covariant derivative that we introduced before: if $V^\mu$ were a vector field $V^\mu(x)$ and we define $V^\mu(\lambda)=V^\mu(x(\lambda))$ to be its value at the point $x(\lambda)$ then

$$
\frac{DV^\mu}{D\lambda}=\frac{dV^\mu}{d\lambda}+\dot{x}^\rho \Gamma_{\rho\,\,\,\nu}^{\,\,\,\mu}V^\nu=\frac{dx^\rho}{d\lambda}\left(\frac{\partial V^\mu}{\partial x^\rho}+\Gamma_{\rho\,\,\,\nu}^{\,\,\,\mu} V^\nu\right)=\frac{dx^\rho}{d\lambda}(\nabla_\rho V^\mu)
$$
...
Definition: let $v^\mu(\tau)=\frac{dx^\mu}{d\tau}$, the four-velocity of a massive particle along a timelike path. We define
...
Before, we saw that in Newtonien gravity, the relative acceleration of two nearby freely-falling particles at $\vec{x}(t)$ and $\vec{x}(t)+\vec{y}(t)$ was given by
...
The relative acceleration is the second proper-time derivative of the separation (we drop the $\delta s$ factor). Then we need to examine the following quantity:
...
Consider an affinely parametrised geodesic $x^\mu(\lambda)$ and the quantity $k=\chi_\mu V^\mu$. $k$ is a scalar quantity, independent of the coordinate choice and the choice of the parameter $\lambda$.
...
We then have for any vector field $W^\mu(x)$ on spacetime
...
If the curves in the congruance are affinely parametrised geodesics, then it is called an affinely parametrised geodesic congruences and $X^\mu=\frac{\partial x^\mu}{\partial\lambda}$ is an auto-parallel vector field
