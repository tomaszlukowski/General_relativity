...
We will take a closer look at geodesics for the Schwarzschild solution. We recall that the Lagrangian is
...
This is because
- we can rotate the coordinate system such that initially we have $\theta=\frac{\pi}{2}$ and $\dot{\theta}=0$
- Lagrangian and initial conditions are symmetric under $\theta\to\pi-\theta$ which means that the orbit remains on the $\theta=\frac{\pi}{2}$ plane. In this case the Lagrangian reduces to
...
Note that the metric has no dependence on $t$ and $\phi$. Therefore there are symmetries

$$
\begin{cases}t\to t+const\\\phi\to\phi+const\end{cases}
$$

and we expect to find two more conserved quantities. These are the **energy** $E$ and the **angular momentum** $J$.
...
The equation for $\dot{r}$ is tricky to solve. Our strategy will be to set $u=\frac{1}{u}$ and look at $\frac{du}{d\phi}=\frac{\dot{u}}{\dot{\phi}}$. Then
...
$$
\boxed{J^2\left(\frac{du}{d\phi}\right)^2+J^2 u^2=E^2-k+\frac{2Mk}{r}+\frac{2MJ^2}{r^3}}\quad\Rightarrow\quad \boxed{\left(\frac{du}{d\phi}\right)^2+u^2=\frac{E^2}{J^2}-\frac{k}{J^2}+\frac{2Mku}{J^2}+2Mu^3}
$$
...
The last term on the right-hand side is coming from General Relativity and was not present there in the solution to this problem in Newtonian physics. We will look at some solutions of this to study how GR makes predictions about solar system physics.
...
$$
\frac{d^2u_2}{d\phi^2}+u_2=3M\sin^2\phi+\mathcal{O}(\frac{1}{b})=\frac{3M}{2}(1-\cos(2\phi))+\mathcal{O}(\frac{1}{b}).
$$

We want a solution which oscillates as pictured symmetrically about $\phi=\frac{\pi}{2}$.
...
$$
\boxed{(-4B\cos(2\phi))+(A+B\cos(2\phi))=\frac{3M}{2}-\frac{3M}{2}\cos(2\phi)}\quad\Rightarrow\quad\boxed{ \begin{cases}A=\frac{3M}{2}\\B=\frac{M}{2}\end{cases}}\quad\Rightarrow\quad \boxed{u_2=\frac{M}{2}(3+\cos(2\phi))}
$$

This means that our overall solution is

$$
u=\frac{1}{b}\sin\phi+\frac{M}{2b^2}(3+\cos(2\phi))+\mathcal{O}(\frac{1}{b^3})
$$
...
Let $\delta$ be the total deflection angle. When $r=\infty$, we have $u=0$ $\phi$ is small when $u=0$. We look at $\phi\approx 0$ and we set $u=0$. Then
...
However, the orbit of Mercuty was observed to rotate (or "precess") so that it does not just repeat the same path for each period.
...
$$
\boxed{\frac{d^2u}{d\phi^2}+u=\frac{1}{l}+3Mu^2}\quad\Rightarrow\quad\boxed{\frac{d^2w}{d\phi^2}+\left(\frac{1}{l}+w\right)=\frac{1}{l}+3M\left(\frac{1}{l}+w\right)^2=\frac{1}{l}+3M\left(\frac{1}{l^2}+\frac{2w}{l}+w^2\right)}
$$
